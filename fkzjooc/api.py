...
class API:
    def __init__(self, phone, password):
        ...
        ocr = ddddocr.DdddOcr()
        # 获取验证码图片
        get_validate_code = self.session.get(
        ...
        # 发起登录请求
        do_login = self.session.post('https://centro.zjlll.net/login/doLogin', data=login_form, verify=False)
        # 登录失败处理
        ...
        # 设置跳转链接，更新会话的Referer
        self.session.get(f'https://www.zjooc.cn/login?time={utils.generateRandomStringWithTimestamp(32)}')
        ...
        user = self.session.get(
            ...
            headers={'Referer': self.referer}, verify=False)
    ...
    def get_course_chapter_list(self, course_id: str) -> dict:
        ...
        chapters = self.session.get(
            url="https://www.zjooc.cn/ajax",
            params={
                "time": utils.generateRandomStringWithTimestamp(32),  # 生成带有时间戳的随机字符串
    ...
    def get_user_video_progress(self, course_id: str, chapter_id: str) -> dict:
        ...
        learning_progress = self.session.get(
            ...
            params={
                "time": utils.generateRandomStringWithTimestamp(32),  # 生成带有时间戳的随机字符串
                "service": "/jxxt/api/course/courseStudent/videoProgress",  # 请求的服务接口
    ...
    def send_course_finish_request(self, course_id: str, chapter_id: str, video_time: int) -> dict:
        """
        发送课程完成请求。
    ...
    def get_test_papers(self, page: int, paper_type: int, course_id: str, batch_key: str):
        """
        获取指定分区下的试卷列表。
        ...
        query_test_papers = self.session.get(
            ...
            params={
                "time": utils.generateRandomStringWithTimestamp(32),  # 生成随机字符串，作为请求的一部分
    ...
    def submit_test_paper(self, paper_id: str, class_id: str, batch_key: str, user_id: str, score_id: str,
                          answers: list):
        """
        提交测试卷答案的函数。
